...
class WBBEmotionDatasetGenerator:
    ...
    def generate_dataset(
        ...
        # 실제 영상 길이 산출 (Step 3의 Sliding Window 범위를 결정하는 값)
        ...
        if video_path and os.path.exists(video_path):
            try:
                ...
                if fps and fps > 0 and frames and frames > 0:
                    video_duration_sec = float(frames) / float(fps)
            except Exception as e:
                print(f"⚠️ 영상 길이 측정 실패({e}) — 채팅 타임스탬프로 대체합니다.")

        if video_duration_sec <= 0:
            # 영상을 못 읽으면 마지막 채팅 시각 + 여유분으로 대체
            if results_list:
                video_duration_sec = float(max(r["timestamp"] for r in results_list)) + 30.0
            else:
                video_duration_sec = 65.0
            print(f"ℹ️ 영상 길이를 채팅 데이터 기준으로 추정: {video_duration_sec:.1f}초")

        meta_data = {
            "total_chats_analyzed": len(df_output),
            "recommended_joy_threshold": round(recommended_threshold, 2),
            # ------------------------------------------------------------------
            # [버그 수정] sliding_window_nlp.py(Step 3)는 아래 두 키를 읽습니다.
            #   - video_metadata.duration_sec
            #   - adaptive_threshold_stats.calculated_threshold
            # 기존에는 이 키들을 쓰지 않아, Step 3가 .get() 폴백값
            # (임계점 8.0, 영상 길이 65초)으로 조용히 동작했습니다.
            # 그 결과 (1) 적응형 동적 임계점이 실제로는 작동하지 않고,
            # (2) 아무리 긴 영상이라도 앞 65초까지만 분석되는 문제가 있었습니다.
            # ------------------------------------------------------------------
            "video_metadata": {
                "duration_sec": round(video_duration_sec, 2)
            },
            "adaptive_threshold_stats": {
                "calculated_threshold": round(recommended_threshold, 2),
                "mean_joy": round(float(mean_joy), 2),
                "std_joy": round(float(std_joy), 2)
            },
